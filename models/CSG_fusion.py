"""Direct conditional fusion variant of CSG.

F = W_out(GELU(W_e(E) + f_ls(ls) + f_topo(topography))).
F is passed directly to the temporal translator. Train this variant from scratch.
"""
...
MODEL_SPEC = {
    "name": "ConvLSTMLsTopographyJointFusionSimVP",
    "description": "ConvLSTM-SimVP with direct GELU fusion of dynamic features, solar longitude and MOLA topography.",
    "parameters": {
        "convlstm_hidden_dim": {
            "type": "int",
            "default": 16,
...
class JointSpatiotemporalFusion(nn.Module):
    ...
    def forward(self, encoded, ls, topography):
        ...
        # Project fused features directly; no bounded gate or multiplicative skip.
...
class ConvLSTMLsTopographyJointFusionSimVP(nn.Module):
    def __init__(
        self,
        in_channels,
        window,
        ...
        self.joint_fusion = JointSpatiotemporalFusion(
    ...
    def forward(self, x, ls, topography):
        ...
        fused = self.joint_fusion(encoded, ls, topography)
...
def build_model(config):
    ...
    fusion_hidden_dim = _positive_int(config, "fusion_hidden_dim")
    ...
    return ConvLSTMLsTopographyJointFusionSimVP(
