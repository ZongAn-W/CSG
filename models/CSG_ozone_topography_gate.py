"""Ozone-only dynamic gate branch plus terrain; all channels remain in backbone.
The ozone index refers to x channel order AFTER platform selection, not dataset ID.
Independent ConvLSTM + spatial encoder generates ozone features for the gate.
"""
import torch
from torch import nn


MODEL_SPEC = {'name': 'CSG_OzoneTopographyGate',
 'description': 'Gate generated from ozone-only history encoding and MOLA; '
                'full-input prediction backbone.',
...
class JointSpatiotemporalGate(nn.Module):
    ...
    def forward(self, encoded, ozone_encoded, topography):
        gate = self.gate_values(ozone_encoded, topography)
...
class ConvLSTMOzoneTopographyGatedSimVP(nn.Module):
    def __init__(
        self,
        in_channels,
        window,
        ...
        super().__init__()
        self.ozone_channel_index = ozone_channel_index
        ...
        # Separate branch prevents other weather channels leaking into gate values.
        self.ozone_convlstm = ConvLSTMEncoder(1, convlstm_hidden_dim)
        self.ozone_spatial_encoder = SpatialEncoder(convlstm_hidden_dim, spatial_hidden_dim)
    ...
    def forward(self, x, topography):
        ...
        ozone = x[:, :, self.ozone_channel_index:self.ozone_channel_index + 1]
        ozone_history = self.ozone_convlstm(ozone)
        ozone_encoded = self.ozone_spatial_encoder(
            ozone_history.reshape(batch * window, ozone_history.shape[2], height, width)
        ).reshape(batch, window, self.spatial_hidden_dim, encoded_height, encoded_width)
        gated, _, _ = self.joint_gate(encoded, ozone_encoded, topography)
...
def build_model(config):
    ...
    ozone_channel_index = config["ozone_channel_index"]
    if (isinstance(ozone_channel_index, bool) or not isinstance(ozone_channel_index, int)
            or not 0 <= ozone_channel_index < in_channels):
        raise ValueError("ozone_channel_index must index the selected input channels [0, in_channels).")
    ...
    return ConvLSTMOzoneTopographyGatedSimVP(
        ...
        initial_gate_strength=initial_gate_strength,
        ozone_channel_index=ozone_channel_index,
    )
