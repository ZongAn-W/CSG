"""Dynamic-topography gate on decoded predictions.
...
MODEL_SPEC = {'name': 'CSG_GateAfterDecoder',
 'description': 'Dynamic-topography gate on decoded predictions.',
...
class ConvLSTMAfterDecoderGatedSimVP(nn.Module):
    def __init__(
        ...
        self.joint_gate = DynamicTopographyGate(
            1, gate_hidden_dim, initial_gate_strength,
        )
    ...
    def forward(self, x, topography):
        ...
        prediction, _, _ = self.joint_gate(prediction, topography)
        return prediction
...
def build_model(config):
    ...
    return ConvLSTMAfterDecoderGatedSimVP(
