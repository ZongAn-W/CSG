"""Dynamic-topography gate between translator and decoder.
...
MODEL_SPEC = {'name': 'CSG_GateBeforeDecoder',
 'description': 'Dynamic-topography gate between translator and decoder.',
...
class ConvLSTMBeforeDecoderGatedSimVP(nn.Module):
    def __init__(
        ...
        self.joint_gate = DynamicTopographyGate(
            spatial_hidden_dim, gate_hidden_dim, initial_gate_strength,
        )
    ...
    def forward(self, x, topography):
        ...
        # Gate future latent frames, using K=horizon rather than T=window.
        translated = translated.reshape(
            batch, self.horizon, self.spatial_hidden_dim, encoded_height, encoded_width,
        )
        translated, _, _ = self.joint_gate(translated, topography)
        translated = translated.reshape(
            batch * self.horizon, self.spatial_hidden_dim, encoded_height, encoded_width,
        )
...
def build_model(config):
    ...
    return ConvLSTMBeforeDecoderGatedSimVP(
