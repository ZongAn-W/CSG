"""Independent dynamic-topography gates between adjacent translator blocks.

Derived from ablations/04_dynamic_topography.py. No Ls or additive FiLM.
G = tanh(W_g(GELU(W_e(F) + f_topo(MOLA)))); F_out = F * (1 + s * G).
s is learnable, initialized to 1, with the original STE [0, 1] clamp.
All backbone modules precede gate construction; Xavier gate gain remains 0.01.
"""
import torch
from torch import nn


MODEL_SPEC = {'name': 'CSG_GateBetweenTranslatorBlocks',
 'description': 'Independent dynamic-topography gates between adjacent translator '
                'blocks.',
...
class DynamicTopographyGate(nn.Module):
    ...
    def gate_values(self, encoded, topography):
        ...
        terrain = self.topography_encoder(topography, (height, width))
        joint_features = features + terrain.to(dtype=features.dtype).unsqueeze(1)
    ...
    def forward(self, encoded, topography):
        gate = self.gate_values(encoded, topography)
...
class BetweenBlocksTranslator(TemporalTranslator):
    """Gate only adjacent block boundaries, never after the last block."""

    def forward(self, x, topography, block_gates):
        x = self.input_projection(x)
        for index, block in enumerate(self.blocks):
            x = block(x)
            if index < len(block_gates):
                # Translator channels already mix history; there is no time axis.
                x, _, _ = block_gates[index](x.unsqueeze(1), topography)
                x = x.squeeze(1)
        return self.output_projection(x)


class ConvLSTMBetweenBlocksGatedSimVP(nn.Module):
    def __init__(
        self, in_channels, window, horizon, convlstm_hidden_dim,
        spatial_hidden_dim, temporal_hidden_dim, num_temporal_blocks,
        dropout, gate_hidden_dim, initial_gate_strength,
    ):
        super().__init__()
        ...
        self.horizon = horizon
        self.spatial_hidden_dim = spatial_hidden_dim
        # Construct the common backbone before gates for matched seeded weights.
        self.convlstm = ConvLSTMEncoder(in_channels, convlstm_hidden_dim)
        self.spatial_encoder = SpatialEncoder(convlstm_hidden_dim, spatial_hidden_dim)
        self.temporal_translator = BetweenBlocksTranslator(
            window, horizon, spatial_hidden_dim, temporal_hidden_dim,
            num_temporal_blocks, dropout,
        )
        self.spatial_decoder = SpatialDecoder(spatial_hidden_dim)
        if num_temporal_blocks < 2:
            raise ValueError("between-block gating requires num_temporal_blocks >= 2.")
        # Each boundary owns its terrain encoder, projections, and strength.
        self.block_gates = nn.ModuleList([
            DynamicTopographyGate(temporal_hidden_dim, gate_hidden_dim, initial_gate_strength)
            for _ in range(num_temporal_blocks - 1)
        ])
    ...
    def forward(self, x, topography):
        ...
        encoded = self.spatial_encoder(
            recurrent_features.reshape(batch * window, recurrent_features.shape[2], height, width)
        )
        encoded_height, encoded_width = encoded.shape[-2:]
        encoded = encoded.reshape(
            batch, window * self.spatial_hidden_dim, encoded_height, encoded_width,
        )
        translated = self.temporal_translator(encoded, topography, self.block_gates)
        translated = translated.reshape(
            batch * self.horizon, self.spatial_hidden_dim, encoded_height, encoded_width,
        )
        decoded = self.spatial_decoder(translated, (height, width))
        prediction = decoded.reshape(batch, self.horizon, 1, height, width)
        return prediction
...
def build_model(config):
    ...
    return ConvLSTMBetweenBlocksGatedSimVP(
