"""ConvLSTM-SimVP with stable per-step physical conditioning.

The primary condition path is the original content-dependent gate: every
historical frame is combined with its own Ls value and shared topography before
the temporal translator. FiLM is an optional, bounded residual ablation and
is disabled by default so the model starts from the proven gated path.
"""
...
class JointSpatiotemporalGate(nn.Module):
    """Apply a bounded, feature-dependent gate independently at each time step."""
...
class SeasonalTerrainCondition(nn.Module):
    """Shared condition used only when the optional FiLM ablation is enabled."""
...
class TemporalInceptionBlock(nn.Module):
    def __init__(self, channels, dropout, condition_hidden_dim, film_strength=0.0):
        super().__init__()
        ...
        # Create the optional branch only when requested so the default model
        # keeps the original parameter count and initialization order.
        self.film_projection = None
        if self.film_strength != 0.0:
            self.film_projection = nn.Conv2d(condition_hidden_dim, 2 * channels, 1)
            nn.init.xavier_uniform_(self.film_projection.weight, gain=0.01)
    ...
    def forward(self, x, condition=None):
        ...
        merged = self.merge(mixed)
        if self.film_strength == 0.0:
            modulated = merged
        else:
            if condition is None:
                raise ValueError("condition is required when film_strength is nonzero.")
            if self.film_projection is None:
                raise RuntimeError("FiLM projection is missing for nonzero film_strength.")
            delta_gamma, beta = self.film_projection(condition).chunk(2, dim=1)
            strength = self.film_strength
            modulated = (1.0 + strength * torch.tanh(delta_gamma)) * merged
            modulated = modulated + strength * torch.tanh(beta)
...
class ConvLSTMLsTopographyBlockFiLMSimVP(nn.Module):
    def __init__(
        ...
        if self.film_strength != 0.0:
            self.condition_encoder = SeasonalTerrainCondition(
                window, condition_hidden_dim
            )
    ...
    def forward(self, x, ls, topography):
        ...
        if self.film_strength != 0.0:
            condition = self.condition_encoder(
                ls, topography, (encoded_height, encoded_width)
            )
...
def _unit_interval(config, key, default):
...
def build_model(config):
    ...
    initial_gate_strength = _unit_interval(config, "initial_gate_strength", 0.05)
    film_strength = _unit_interval(config, "film_strength", 0.0)
