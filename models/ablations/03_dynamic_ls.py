"""A3: dynamic_ls ablation of original CSG, not the fusion/FiLM variants.
...
MODEL_SPEC = {'name': 'CSG_A3_dynamic_ls',
 'description': 'Original CSG ablation: dynamic_ls; learnable gate strength '
                'initialized to 1.',
 'parameters': {'convlstm_hidden_dim': {'type': 'int',
                ...
                'initial_gate_strength': {'type': 'float',
                                          ...
                                          'min': 0.0,
                                          'max': 1.0}},
 'auxiliary_inputs': {'ls': {'required': True,
                             'shape': ['batch', 'window'],
                             'dtype': 'float32',
                             'unit': 'degree'}}}
...
class JointSpatiotemporalGate(nn.Module):
    ...
    def gate_values(self, encoded, ls=None, topography=None):
        ...
        joint_features = joint_features + self.ls_encoder(ls).to(dtype=features.dtype).unsqueeze(-1).unsqueeze(-1)
...
class ConvLSTMLsTopographyJointGatedSimVP(nn.Module):
    ...
    def forward(self, x, ls):
        ...
        gated, _, _ = self.joint_gate(encoded, ls=ls, topography=None)
