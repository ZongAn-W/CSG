...
MODEL_SPEC = {
    "name": "ConvLSTMStateMotionSimVP",
    "description": (
        "Complete-history ConvLSTM-SimVP with an additive encoded-state "
        "difference stream."
    ),
    "parameters": {
        "convlstm_hidden_dim": {
...
def _build_motion(state):
    first = torch.zeros_like(state[:, :1])
    differences = state[:, 1:] - state[:, :-1]
    return torch.cat((first, differences), dim=1)
...
class StateMotionTranslator(nn.Module):
    def __init__(
        self,
        window,
        horizon,
        ...
        super().__init__()
        input_channels = window * spatial_hidden_dim
        self.state_projection = nn.Conv2d(
            input_channels, temporal_hidden_dim, kernel_size=1
        )
        self.motion_projection = nn.Parameter(
            torch.zeros(temporal_hidden_dim, input_channels, 1, 1)
        )
    ...
    def forward(self, state_flat, motion_flat):
        state_latent = self.state_projection(state_flat)
        motion_latent = torch.nn.functional.conv2d(
            motion_flat, self.motion_projection
        )
        latent = state_latent + motion_latent
        latent = self.blocks(latent)
...
class ConvLSTMStateMotionSimVP(nn.Module):
    def __init__(
        self,
        in_channels,
        window,
        ...
        self.temporal_translator = StateMotionTranslator(
            window,
            horizon,
            spatial_hidden_dim,
            temporal_hidden_dim,
    ...
    def forward(self, x):
        ...
        translated = self.temporal_translator(state_flat, motion_flat)
...
def build_model(config):
    ...
    return ConvLSTMStateMotionSimVP(
